...
namespace logger {
    ...
    void log_ray_directions(int camera_id, int frame_index, int pixel_x, int pixel_y, const Eigen::Vector3f& camera_pos, const Eigen::Vector3f& ray_dir) {
        if (!g_log_file) return;

        if (camera_id != g_last_camera_id) {
            g_log_file << "-----------------------\n";
            g_log_file << "Cam: " << camera_id << ", Position: ["
                << camera_pos.x() << ", "
                << camera_pos.y() << ", "
                << camera_pos.z() << "]\n";
        ...
        g_log_file << "     Frame: " << std::setw(4) << std::right << frame_index
            << ", Object at: (" << std::setw(4) << std::right << pixel_x << ", " << std::setw(4) << std::right << pixel_y << ")"
            << ", Ray Direction: [" << std::setw(10) << std::right << std::setprecision(6) << ray_dir.x() << " "
            << std::setw(10) << std::right << std::setprecision(6) << ray_dir.y() << " "
            << std::setw(10) << std::right << std::setprecision(6) << ray_dir.z() << "]"
            << '\n';
    }

    void log_formatted_ray(const Eigen::Vector3f& camera_pos, const Eigen::Vector3f& unit_vector, float scale_factor, int camera_id, int frame_index) {
        if (!g_formatted_log_file) return;


        if (camera_id != g_last_formatted_camera_id) {
            g_formatted_log_file << "-----------------------\n";
            g_formatted_log_file << "Cam: " << camera_id << ", Position: ["
                << std::fixed << std::setprecision(6) << camera_pos.x() << ", "
        ...
        g_formatted_log_file << "     Frame: " << std::setw(4) << std::right << frame_index
            ...
            << std::fixed << std::setprecision(6) << camera_pos.y() << ", "
            << std::fixed << std::setprecision(6) << camera_pos.z() << "), ("
            << std::fixed << std::setprecision(3) << endpoint.x() << ", "
            << std::fixed << std::setprecision(3) << endpoint.y() << ", "
            << std::fixed << std::setprecision(3) << endpoint.z() << "))\n";
    }

    void shutdown() {
        if (g_log_file.is_open()) g_log_file.close();
        if (g_formatted_log_file.is_open()) g_formatted_log_file.close();
        g_last_camera_id = -1;
