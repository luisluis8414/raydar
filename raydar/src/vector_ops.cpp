...
Eigen::Vector3f get_ray_direction(const FrameInfo& info, int pixel_x, int pixel_y, int img_width,
                                  int img_height) {
    ...
    float camera_y =
        (1.0f - 2.0f * (pixel_y + 0.5f) / img_height) * tan_half_horizontal_fov * aspect_ratio;
    float camera_z = -1.0f;
