...
std::map<int, std::vector<FrameInfo>>
load_metadata(const std::string &metadata_file) {
  ...
  try {
    ...
    for (const nlohmann::json &frame : json_data) {
      ...
      std::vector<raydar::FrameInfo>::iterator insert_pos = std::lower_bound(
          frames.begin(), frames.end(), info,
...
DetectionArray detect_motion(const Image &prev_img, const Image &curr_img,
                             const DetectionArray &prev_detection,
  ...
  for (size_t i = 0; i < prev_detection.pixels_with_motion.size(); i++) {
    ...
    prev_detection_set.insert(PixelChange{
        prev_pixel_change.x, prev_pixel_change.y, prev_pixel_change.change});
  }
  ...
  for (int y = 0; y < curr_img.height; y++) {
    for (int x = 0; x < curr_img.width; x++) {
      ...
      if (abs(change) > motion_threshold) {
        ...
        if (prev_detection_set.find(current_pixel) ==
            prev_detection_set.end()) {
    ...
    while (!q.empty()) {
      ...
      for (int dx = -2; dx <= 2; ++dx) {
        for (int dy = -2; dy <= 2; ++dy) {
          ...
          // the object
          if (unprocessed_pixels.count(
                  neigh)) { // no bound checks needed, all pixels in
...
SkewResult closest_point_between_rays(const Eigen::Vector3f &p,
...
std::map<int, std::vector<Eigen::Vector3f>> compute_3d_points(
  ...
  for (std::map<int,
                std::map<int, std::vector<Eigen::Vector3f>>>::const_iterator
           cam = rays.begin();
       cam != rays.end(); ++cam) {
    for (std::map<int, std::vector<Eigen::Vector3f>>::const_iterator frm =
             cam->second.begin();
         frm != cam->second.end(); ++frm) {
      if (!frm->second.empty()) {
  ...
  for (std::set<int>::const_iterator frame_it = all_frames.begin();
       frame_it != all_frames.end(); ++frame_it) {
    int frame = *frame_it;
    ...
    for (std::map<int,
      ...
      std::map<int, std::vector<Eigen::Vector3f>>::const_iterator it =
          cam->second.find(frame);
      if (it != cam->second.end() && !it->second.empty()) {
        std::map<int, std::map<int, Eigen::Vector3f>>::const_iterator pos_it =
            camera_positions.find(cam_id);
        if (pos_it != camera_positions.end()) {
          std::map<int, Eigen::Vector3f>::const_iterator frm_pos =
              pos_it->second.find(frame);
    ...
    struct RecurseHelper {
      static void
      recurse(const std::vector<std::vector<Eigen::Vector3f>> &frame_rays,
        ...
        if (cam_idx == N) {
          ...
          for (size_t a = 0; a < static_cast<size_t>(N) && is_valid; ++a) {
            for (size_t b = a + 1; b < static_cast<size_t>(N) && is_valid;
                 ++b) {
          ...
          if (is_valid && !midpoints.empty()) {
            ...
            for (std::vector<Eigen::Vector3f>::const_iterator m =
                     midpoints.begin();
                 m != midpoints.end(); ++m) {
              sum += *m;
            }
        ...
        for (int r = 0; r < num_rays_per_cam[cam_idx]; ++r) {
          choice[cam_idx] = r;
          recurse(frame_rays, frame_pos, num_rays_per_cam, min_distance,
                  valid_points, N, cam_idx + 1, choice);
        }
      }
    };
    std::vector<int> choice(N, 0);
    RecurseHelper::recurse(frame_rays, frame_pos, num_rays_per_cam,
                           min_distance, valid_points, static_cast<int>(N), 0,
                           choice);
...
void write_3d_points_to_file(
    const std::map<int, std::vector<Eigen::Vector3f>> &frame_points,
  ...
  for (std::map<int, std::vector<Eigen::Vector3f>>::const_iterator it =
           frame_points.begin();
...
void generate_flight_path_images(
  ...
  for (std::map<int, std::vector<FrameInfo>>::const_iterator cam =
           camera_frames.begin();
...
void detect_objects(const std::string &metadata_file_path,
                    float detect_motion_threshold, float min_distance) {
  ...
  std::map<int, std::vector<FrameInfo>> camera_frames =
      load_metadata(metadata_file_path);
  std::map<int, std::map<int, std::vector<Eigen::Vector3f>>> rays;
  std::map<int, std::map<int, Eigen::Vector3f>> camera_positions;
  std::map<int, std::vector<std::pair<int, int>>>
      all_object_centers; // for flight path

  for (std::map<int, std::vector<raydar::FrameInfo>>::const_iterator
           camera_frames_pair = camera_frames.begin();
    ...
    if (frames.size() < 2) {
      std::cerr << "WARNING: Camera " << camera_id
                << " has less than 2 frames, skipping" << std::endl;
      continue;
    }
    ...
    for (size_t i = 0; i < frames.size(); i++) {
      ...
      DetectionArray detection_array = detect_motion(
          *prev_img, curr_img, prev_detection, detect_motion_threshold);
      prev_detection = detection_array;

      std::vector<std::pair<int, int>> object_centers =
          find_object_centers(detection_array);
...

      {
        ...
        std::string output_name =
            "motion_camera" + std::to_string(curr_info.camera_index) +
            "_frame" + std::to_string(curr_info.frame_index) + ".png";
        std::string output_path = output_dir + "/" + output_name;
        std::filesystem::create_directories(output_dir);
        visualize_motion(curr_img, detection_array, output_path,
                         object_centers);
      }
      ...
      for (const std::pair<int, int> &center : object_centers) {
        ...
        camera_positions[camera_id][curr_info.frame_index] =
            curr_info.camera_position;

        logger::log_ray_directions(camera_id, curr_info.frame_index, x, y,
                                   curr_info.camera_position, dir);
        logger::log_formatted_ray(curr_info.camera_position, dir, 2000,
                                  camera_id, curr_info.frame_index);
      }
  ...
  std::map<int, std::vector<Eigen::Vector3f>> frame_points =
      compute_3d_points(rays, camera_positions, min_distance);
