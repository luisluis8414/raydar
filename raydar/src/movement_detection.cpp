...
namespace raydar {
    ...
    std::vector<RaysAtFrame> group_rays_by_frame(
        ...
        std::sort(frame_rays.begin(), frame_rays.end(),
                  [](const RaysAtFrame &a, const RaysAtFrame &b) {
        ...
        struct RecurseHelper {
            static void recurse(
                ...
                for (int r = 0; r < num_rays_per_cam[cam_idx]; ++r) {
                    choice[cam_idx] = r;
                    recurse(cam_rays, cam_pos, num_rays_per_cam, min_distance, valid_points, N, cam_idx + 1, choice);
                }
            }
        };

        std::vector<int> choice(N, 0);
        RecurseHelper::recurse(cam_rays, cam_pos, num_rays_per_cam, min_distance, valid_points, N, 0, choice);
        return valid_points;
    }

     // compute 3d coords from ray intersections across cameras at same frame (frame means a snapshot where all cameras capture the same scene)
    std::map<int, std::vector<Eigen::Vector3f>> compute_3d_points(const std::vector<RaysAtFrame> &frame_rays, float min_distance) {
        ...
        for (const auto &frame : frame_rays) {
            ...
            if (N < 2) continue; // skip if less than 2 cameras (no intersection possible)

            std::vector<std::vector<Eigen::Vector3f>> cam_rays(N); // rays grouped by active camera index
            ...
            std::vector<Eigen::Vector3f> valid_cords = find_valid_coords(cam_rays, cam_pos, num_rays_per_cam, min_distance, static_cast<int>(N));
    ...
    void detect_objects(const std::string &metadata_file_path,
        ...
        // convert camera centered data to frame-centered structure => more efficient for datastructure for compute_3d_points
        std::vector<RaysAtFrame> frame_rays = group_rays_by_frame(rays, camera_positions);
        std::map<int, std::vector<Eigen::Vector3f>> frame_points = compute_3d_points(frame_rays, min_distance);
